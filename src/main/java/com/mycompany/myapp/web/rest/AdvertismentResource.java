...
/**
 * REST controller for managing {@link com.mycompany.myapp.domain.Advertisment}.
 */
@RestController
@RequestMapping("/api")
public class AdvertismentResource {
    ...
    public ResponseEntity<List<Advertisment>> getAllAdvertisments(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("address-is-null".equals(filter)) {
            log.debug("REST request to get all Advertisments where address is null");
            return new ResponseEntity<>(advertismentService.findAllWhereAddressIsNull(),
                    HttpStatus.OK);
        }
        if ("feature-is-null".equals(filter)) {
            log.debug("REST request to get all Advertisments where feature is null");
            return new ResponseEntity<>(advertismentService.findAllWhereFeatureIsNull(),
                    HttpStatus.OK);
        }
